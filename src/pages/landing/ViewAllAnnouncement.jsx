...
const ViewAllAnnouncement = () => {
  ...
  // Function for View Announcement Pop Up Modal
  const viewAll = (index) => {
  ...
  return(
    <>
      ...
      {/* View All Announcement Pop Up Modal */}
      {dataVisible !== null && 
        <section 
        className="fixed flex justify-center items-center w-full h-screen inset-0 z-[9999] bg-black/50 px-4 cursor-pointer"
        // onClick={(e) => {e.target === e.currentTarget ? setDataVisible(null) : setDataVisible(currentCards[dataVisible])}}
        > 

          {currentCards[dataVisible] && (
            <div className="flex flex-col w-4xl h-[90%] ">
              <header className={`flex flex-col w-full p-6 md:p-12 gap-4 rounded-t-lg ${currentCards[dataVisible].bgColor}`}  >
                <div className="flex justify-between items-center w-full text-sm md:text-base text-white font-semibold">
                  ...
                  <button
                    className="flex gap-2 items-center w-fit cursor-pointer text-sm text-white bg-red-500 hover:bg-red-600 active:bg-red-700 pl-4 pr-5 py-2 rounded-4xl transition-all shadow-sm group"
                    onClick={() => viewAll(dataVisible)}
                  >
                    <ArrowLeft className="h-4 w-4 group-hover:-translate-x-2 transition-all"/>
                    Back
                  </button>      
                </div>
              ...
              <footer className="flex flex-col bg-white p-6 md:p-12 gap-8 rounded-b-lg overflow-y-auto">
                ...
                <span className="text-gray-500 texy-justify">
                  {currentCards[dataVisible].desc}
                </span>

                <span className="break-normal text-gray-500 text-justify">
